   ...
   function authSuccess(id,token) {
	...
	function addOverEvent(){
		...
		$('#pageNav').click(function(e) {
			...
			if(data==">")
			{
				if(currentGrandPage>=grandPageTotal)
					return;
	...
	function hideSearchBook(){
		searchData=null;
		cst_cst_clearNodeById('searchResultContainer');
		$('#myModal').modal('hide');
	}
	...
	function showPopup(target){
				var popup;
						...
						{
							popup = newchromeLess("./popup/imgdelete.html?img="+img,300,150,200,200, "regImg");
						}else{
	...
	function clearNewItme(){
		...
		$('#img1').attr("src", receivedata.cover_l_url);
		$('#img2').attr("src", receivedata.cover_l_url);
		$('#img3').attr("src", receivedata.cover_l_url);
		$('#img4').attr("src", receivedata.cover_l_url);
		$('#submitPrice').attr("value",receivedata.sale_price);
		$('#submitTitle').attr("value",receivedata.title);
		$('#submitText').text("");
	}
	...
	function submitNewItem(e){
		var receivedata=selectedBookItem;
			$('#submitIme').click(function(e) {
				getData(function(usrdata){
				var data={};
				data.owner=usrdata.id;
				data.image=$('#img1').attr("src");
				data.imgDetail=$('#img2').attr("src")+"&"+$('#img3').attr("src")+"&"+$('#img4').attr("src");
				data.price=$('#submitPrice').attr("value");
				data.booktitle = receivedata.title  ? receivedata.title : "";
				data.title= $('#submitTitle').attr("value");
				data.isbn=receivedata.isbn ? receivedata.isbn : "";
				data.quality= $(":input:radio[name=sample]:checked").val();
				data.publisher =receivedata.pub_nm? receivedata.pub_nm : "";
				data.desc =$('#submitText').text();
				alert(data);
				});
			});
	}
